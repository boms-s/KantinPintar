...
import { Pencil, X, Clock3, MapPin, Upload } from "lucide-react";
...
export default function ProfilPage() {
  ...
  const [schedule, setSchedule] = useState<OperatingHoursSchedule>({ ...EMPTY_SCHEDULE });
  ...
  useEffect(() => {
    const loadSession = async () => {
      ...
      if (current.penjual) {
        ...
        // Parse operatingHours: JSON per-hari atau string lama
        const parsed = parseOperatingHours(current.penjual.operatingHours);
        setSchedule(parsed ?? { ...EMPTY_SCHEDULE });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Update profile
      const serializedHours = serializeOperatingHours(schedule);
      const result = await updatePenjualProfileAction({
        description: description.trim() || undefined,
        operatingHours: serializedHours,
        isOpen,
          ...
          ? {
              ...
              penjual: current.penjual
                ? {
                    ...current.penjual,
                    description,
                    operatingHours: serializeOperatingHours(schedule),
                    isOpen,
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <div className="col-span-2 space-y-6">
            ...
            {/* Jam Operasional */}
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow">
              <h3 className="text-base font-semibold text-slate-900 mb-3">Jam Operasional</h3>
              {(() => {
                const parsed = parseOperatingHours(sellerProfile?.operatingHours);
                if (parsed) {
                  return (
                    <div className="space-y-2">
                      {DAY_KEYS.map((day) => (
                        <div key={day} className="flex items-center justify-between text-sm">
                          ...
                          <span className={parsed[day] ? "font-medium text-slate-800" : "text-slate-400"}>
                            {parsed[day] || "Libur"}
                          </span>
                        </div>
                      ))}
                ...
                return (
                  <div className="flex items-center gap-3 text-sm text-slate-600">
                    ...
                    <span>{sellerProfile?.operatingHours || "Belum ditentukan"}</span>
                  </div>
                );
              })()}
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <p className="mb-3 text-xs text-slate-500">
                  Isi format <span className="font-mono bg-slate-100 px-1 rounded">HH:MM-HH:MM</span> untuk setiap hari.
                  Kosongkan jika libur.
                </p>
                  ...
                  {DAY_KEYS.map((day) => (
                    <div key={day} className="flex items-center gap-3">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSchedule((prev) => ({ ...prev, [day]: e.target.value }))
                        }
